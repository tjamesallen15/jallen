...
import type { Heading } from "../../../../shared/common/page";
import type {
  About,
  Experience,
  Information,
    ...
    >
      <div className="container mx-auto">
        <Tabs
          ...
          className="flex flex-col gap-[20px] xl:flex-row xl:gap-[60px]"
        >
          <TabsList className="flex flex-row w-full max-w-[380px] mx-auto xl:flex-col xl:mx-0 bg-transparent">
            <TabsTrigger value={profileHeading.value}>
              {profileHeading.title}
            </TabsTrigger>
            <TabsTrigger value={experienceHeading.value}>
              {experienceHeading.title}
            </TabsTrigger>
            <TabsTrigger value={skillsHeading.value}>
              {skillsHeading.title}
            </TabsTrigger>
              ...
              {informations?.map((item, index) => {
                return (
                  ...
                  >
                    ...
                    <div className="font-karla text-foreground">
                      {item.value}
                    </div>
                  </div>
                );
              ...
              <div className="flex flex-col gap-[30px]">
                <div className="flex flex-col gap-[10px] text-center xl:text-left">
                  ...
                  <p className="max-w-[600px] mx-auto xl:mx-0">
                    {profileHeading.description}
                  </p>
                </div>

                <ScrollArea className="h-[400px]">
                  {aboutLoading && (
                    <Skeleton containerClassName="w-full" count={12} />
                  )}
                  <div className="flex flex-col gap-2 bg-sidebar rounded-md">
                    {abouts.map((item: About, index: number) => {
                      return (
                        <p
                          className="font-karla pt-4 ps-6 pe-6 text-primary"
                          key={index}
                        >
                          {item.description}
                        </p>
                  ...
                  <ul className="flex flex-col gap-4">
                    ...
                    {experiences.map((item: Experience, index: number) => {
                      return (
                        ...
                        >
                          <h3 className="text-primary text-xl font-semibold max-w-[400px] min-h-[20px] text-center lg:text-left">
                            {item.company}
                          </h3>
                          <h4 className="text-primary">{item.position}</h4>
                          <span className="text-link-accent text-sm">
                            {item.duration}
                          </span>
                          ...
                          <div className="mt-2">
                            <p className="font-karla text-primary">
                              {item.description}
                            </p>
                          </div>
                        </li>
